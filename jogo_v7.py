...
# Função para criar uma nova torre
def criar_torre(pos_x, pos_y):
    nova_torre = Torre(10, 100, 150, mago_img)  # Exemplo de criação de torre
    nova_torre.rect.center = (pos_x, pos_y)  # Define a posição da torre
    torres.add(nova_torre)  # Adiciona a torre ao grupo de torres
...
while teste:
    ...
    # Verifica se alguma torre atinge algum inimigo
    for torre in torres:
        for inimigo in fantasmas:
            if torre.rect.colliderect(inimigo.rect):
                # Implemente a lógica de ataque da torre ao inimigo aqui
                pass  # Exemplo: inimigo.vida -= torre.dano
